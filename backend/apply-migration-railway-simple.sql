-- ============================================
-- MIGRACIÓN SIMPLIFICADA PARA RAILWAY (con UUID)
-- ============================================
...
CREATE UNIQUE INDEX "membership_members_membership_id_member_id_key" ON "membership_members"("membership_id", "member_id");
CREATE INDEX "membership_members_gym_id_idx" ON "membership_members"("gym_id");
CREATE INDEX "membership_members_membership_id_idx" ON "membership_members"("membership_id");
CREATE INDEX "membership_members_member_id_idx" ON "membership_members"("member_id");
CREATE INDEX "membership_members_is_primary_idx" ON "membership_members"("is_primary");
CREATE INDEX "memberships_pricing_plan_id_idx" ON "memberships"("pricing_plan_id");

-- Paso 7: Agregar foreign keys
ALTER TABLE "membership_members"
  ADD CONSTRAINT "membership_members_gym_id_fkey"
    FOREIGN KEY ("gym_id") REFERENCES "gyms"("id") ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE "membership_members"
  ADD CONSTRAINT "membership_members_membership_id_fkey"
    FOREIGN KEY ("membership_id") REFERENCES "memberships"("id") ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE "membership_members"
  ADD CONSTRAINT "membership_members_member_id_fkey"
    FOREIGN KEY ("member_id") REFERENCES "members"("id") ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE "memberships"
  ADD CONSTRAINT "memberships_pricing_plan_id_fkey"
    FOREIGN KEY ("pricing_plan_id") REFERENCES "pricing_plans"("id") ON DELETE SET NULL ON UPDATE CASCADE;

-- Verificación
SELECT 'Migración completada exitosamente' as status;
SELECT COUNT(*) as "Membresías migradas" FROM membership_members;
